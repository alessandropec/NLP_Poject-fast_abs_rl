...
def lstm_encoder(sequence, lstm,import torch
from torch import nn
from torch.nn import functional as F
from torch.nn import init
...
def lstm_encoder(sequence, lstm,
    ...
    if seq_lens: # number of sentences per report in the batch
    ...
    if seq_lens: # number of sentences per report in the batch
        # we use pack_padded_sequence and pad_packed_sequence because tensors in a tensor must have the same size
